...
def get_event_zscore(evt_type, s_utc, e_utc, acc_x, acc_y, spd):
    ...
    for i in range(dt_num):
        ...
        spd_bin[i] = dst.spd_bins(spd.loc[spd.index[s_idx+sec_idx[i]*stepSize]:spd.index[s_idx+sec_idx[i+1]*stepSize]].mean())
    ...
    return spd_bin, acc_z, dec_z, lat_lt_z, lat_rt_z
...
def event_eva(acc_x, acc_y, spd, df_evt):
    ...
    df_evaluation = pd.DataFrame(np.nan, index=np.arange(1000), columns=['type','d','s_utc','e_utc',
                       ...
                       'sec1_spd_bin','sec1_acc_z','sec1_dec_z','sec1_lat_lt_z','sec1_lat_rt_z',\
                       'sec2_spd_bin','sec2_acc_z','sec2_dec_z','sec2_lat_lt_z','sec2_lat_rt_z',\
                       'sec3_spd_bin','sec3_acc_z','sec3_dec_z','sec3_lat_lt_z','sec3_lat_rt_z'])  

    evt_num = df_evt.shape[0]

    for i in range(evt_num):    
        spd_bin, acc_z, dec_z, lat_lt_z, lat_rt_z = get_event_zscore(df_evt['type'][i], df_evt['s_utc'][i], df_evt['e_utc'][i], acc_x, acc_y, spd)
